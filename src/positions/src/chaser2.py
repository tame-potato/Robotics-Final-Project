...
class Chaser:
    def __init__(self):
        # ------------ Subscriptions -------------
        self.sub = rospy.Subscriber('/robot_1/odom', Odometry, self.saveOdomSelf)       # chaser Team B
...
#        self.sub = rospy.Subscriber('/robot_7/odom', Odometry, self.saveOdomRobot7)     # keeper Team B
        self.sub = rospy.Subscriber('/robot_13/odom', Odometry, self.saveOdomRobot13)     # quaffle
        self.sub = rospy.Subscriber('/robot_12/odom', Odometry, self.saveOdomRobot12)   # goal Team A (team B scores here)
...
#        self.robot_7_odom = Odometry()      # keeper Team B odometry
        self.robot_13_odom = Odometry()      # quaffle2 odometry
        self.robot_12_odom = Odometry()     # goal Team A odometry
        self.twist = Twist()    
...
#        self.robot_7_odom = msg
    def saveOdomRobot13(self, msg):      # robot_13 quaffle2
        self.robot_13_odom = msg
    def saveOdomRobot12(self, msg):     # robot_11 Goal Team B
        self.robot_12_odom = msg
        # ----------- end of Callbacks ------------
...
#        print('Bearing:', bearing)
    ...
    def got_the_quaffle(self):
        ...
        x1 = self.my_odom.pose.pose.position.x  # self  Robot_1 chaser Team B                                             
        y1 = self.my_odom.pose.pose.position.y  # self  Robot_1 chaser Team B                                               
        x2 = self.robot_13_odom.pose.pose.position.x  # quaffle2 Robot_13
        y2 = self.robot_13_odom.pose.pose.position.y  # quaffle2 Robot_13
    ...
    def check_if_scored(self):
        ...
        x2 = self.robot_12_odom.pose.pose.position.x    # Robot_12 Goal Team B
        y2 = self.robot_12_odom.pose.pose.position.y    # Robot_12 Goal Team B
        distance=self.check_distance(x1, y1, x2, y2)
    ...
    def process(self):
        ...
        if self.gotthequaffle_flag:         # quaffle needs to follow
            ...
            self.goto_location(self.robot_12_odom)     # move to goal Team B Robot_12
            ...
            self.goto_location(self.robot_13_odom)      # move to quaffle2 Robot_13
