...
class EvenKeeper:
    def __init__(self):
        ...
        self.robot_6_odom = Odometry()                                                    # initialize messages for odometries and twist motion
        self.robot_1_odom = Odometry()
        self.robot_12_odom = Odometry()
        self.twist = Twist()
    ...
    def saveOdomSelf(self, msg):                                                          # save own (Even Keeper's) odometry
        self.robot_6_odom = msg

    def saveOdomRobot1(self, msg):                                                        # save Odd Chaser's odometry
        self.robot_1_odom = msg

    def saveOdomRobot12(self, msg):                                                       # save Even Goal's odometry
    ...
    def process(self):
        ...
        while self.BlockedChaser is False:
            ...
            if (dist_bw_chaser_goal > 10):
                ...
                evenkeeper.updatePosition()
                dist_bw_keeper_chaser = self.check_distance(self.keeper_position[0], self.keeper_position[1], self.chaser_position[0], self.chaser_position[1])
                while (dist_bw_keeper_chaser > 0):                                       # while Even Keeper hasn't reached Odd Chaser
                    self.twist.linear.x = 2                                              # move towards Odd Chaser
                    self.twist.angular.z = self.get_twist_to_waypoint(self.robot_6_odom, self.robot_1_odom)
                    self.pub.publish(self.twist) 
                    time.sleep(0.2) 
                    evenkeeper.updatePosition()
                    dist_bw_keeper_chaser = self.check_distance(self.keeper_position[0], self.keeper_position[1], self.chaser_position[0], self.chaser_position[1])
...
if __name__ == '__main__':
    rospy.init_node('evenkeeper_node', anonymous=True)
