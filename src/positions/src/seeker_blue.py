...
class Seeker:
    def __init__(self):
        self.sub = rospy.Subscriber('/robot_5/odom', Odometry, self.saveOdomSelf)	#seeker for Gryffindor aka HP
        self.sub = rospy.Subscriber('/robot_10/odom', Odometry, self.saveOdomRobot10)		#Snitch 
        self.sub = rospy.Subscriber('/robot_4/odom', Odometry, self.saveOdomSelf)		#Hufflepuff seeker (rival robot)

        self.pub = rospy.Publisher('/robot_5/cmd_vel', Twist, queue_size=10)	#publishes the move command for HP seeker bot
...
#Begin Rotations and Distance Calculations
        y1 = self.my_odom.pose.pose.position.x
