...
class Quaffle:
    def __init__(self):
        # ------------ Subscriptions -------------
        self.sub = rospy.Subscriber('/robot_13/odom', Odometry, self.saveOdomSelf)      # quaffle
        self.sub = rospy.Subscriber('/robot_0/odom', Odometry, self.saveOdomRobot0)     # chaser Team A
        self.sub = rospy.Subscriber('/robot_1/odom', Odometry, self.saveOdomRobot1)     # chaser Team B
        ...
        self.pub = rospy.Publisher('/robot_13/cmd_vel', Twist, queue_size=10)
    ...
    def get_x_and_y_from_robots(self):
        ...
        if self.dist(x1, y1, x2, y2) > 4:
            self.acquired = None
        elif self.dist(x1, y1, x2, y2) < 2 or self.acquired == 'red':
            #print('Red acquired')
            self.acquired = 'red'
        ...
        if self.dist(x1, y1, x2, y2) > 4:
            self.acquired = None
        elif self.dist(x1, y1, x2, y2) < 2 or self.acquired == 'blue':
            #print('Red acquired')
            self.acquired = 'blue'
